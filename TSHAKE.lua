...
if text == 'يونس' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'هاذا حبيبي وغلاي وكلشي�❤️'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'حسين' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'هاذا اليدر مال اني احبه حياتي حسىن�🏃'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'يويو' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'مطوري تاج راسي هوا صنعني�💔🍃'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'فهد' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'هاذا اليدر مال اني احبه فهودي😻😹'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
if text == 'صادق' then 
if not database:get('bot:rep:mute'..msg.chat_id_) then
moody = 'هاذا اليدر مال اني احبه صدوقي'
else 
moody = ''
end
send(msg.chat_id_, msg.id_, 1, moody, 1, 'md')
...
send(msg.chat_id_, msg.id_, 1, '● - `تم وضع الكليشه بالتحذير للعدد` ✓⬅️ : *'..sensspam[2]..'*', 1, 'md')
	...
	if wel then
         ...
         send(msg.chat_id_, msg.id_, 1, '● - `لم يتم وضع ��رحيب للمجموعه` ⚠️', 1, 'md')
end
	end
	end
	-----------------------------------------------------------------------------------------------
...
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   ...
   if lockpt[2] == "bots" or TSHAKEPT[2] == "البوتات" then
    ...
    if database:get('lang:gp:'..msg.chat_id_) then
         ...
         send(msg.chat_id_, msg.id_, 1, '● - `بالفعل تم` ✔️ `ف��ح كــل الوسائط` 🔓\n\n● - `خاصية : المسح` ⚠️', 1, 'md')
      end
      end
      end
...
local user_msgs = database:get('user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
   if result.photos_[0] then
      ...
      t = 'مدير الكروب تاج مخي❗️'
      end
      elseif is_mod(msg.sender_user_id_, msg.chat_id_) then
      if database:get('lang:gp:'..msg.chat_id_) then
      t = 'Group Moderator'
      else
      t = 'ادمن بالكروب تاج راسي 🎐'
      end
      else
      if database:get('lang:gp:'..msg.chat_id_) then
      t = 'Group Member'
      else
      t = 'عضو لا تحل ولا تربط ⚠️'
      end
    end
          if database:get('lang:gp:'..msg.chat_id_) then
...
if text:match("^source$") or text:match("^اصدار$") or text:match("^الاصدار$") or text:match("^السورس$") or text:match("^سورس$") then
   ...
   local text =  [[
<code>اهلا بك في سورس يويو</code>
...
<b>Dev | </b>@yhbvn1
<b>Dev | </b>@hasseneo
<b>Dev | </b>@Youyou_toisl_bot

<code>قناه السورس : </code>

<b>Channel | </b>@DEV_youyou

<code>تم صنع البوت من قبل المارينز يونس :</code>
